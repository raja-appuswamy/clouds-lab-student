...
**Environment: Google Colab** with a **GPU (T4)**
runtime — *not* Cloud Shell (there is no free GPU on GCP; all training is in Colab).
...
small text corpus (TinyShakespeare), then benchmark across CPU / threads / GPU.
...
the provided **Colab notebook** drives four runs — CPU-naive, CPU-vectorized, CPU-thread
sweep, GPU — plots the results, trains the model, uploads the weights to **Cloud
...
host↔device transfer overhead for small batches, when the GPU is starved.)
