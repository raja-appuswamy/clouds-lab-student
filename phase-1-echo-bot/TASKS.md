...
- [ ] **2. Implement the code, run the unit tests.**
  Fill the TODOs in [app.py](app.py) (`build_echo`, `extract_message`) and the two TODO
  lines in the [Dockerfile](Dockerfile). Then, offline:
  ```bash
...
- [ ] **4. IaaS path — run the container on an `e2-micro` VM.**
  ```bash
...
- [ ] **5. Container PaaS path — deploy the same image to Cloud Run.**
  ```bash
...
- [ ] **6. Serverless function path — deploy `main.py` as a Cloud Function (gen 2).**
  ```bash
...
- [ ] **7. Measure all three and write the report.**
  ```bash
...
- [ ] **8. Commit, push, confirm green CI.**
...
- [ ] **9. Write the comparison report** (deliverable below).

- [ ] **10. Tear down** (after you're graded) to protect your quota:
  ```bash
